...
class ToolsList extends HTMLElement {
  ...
  connectedCallback () {
    ...
    this.toolsImporter = this.shadowRoot.querySelector('#importer');
    this.toolsImportButton = this.shadowRoot.querySelector('button#import');
  }
  ...
  populate (state) {
    ...
    this.toolsImportButton.addEventListener('click', this.import.bind(this));
  ...
  import () {
    const schemaData = this.toolsImporter.value;
    const tools = FunctionTool.importAll(schemaData);
